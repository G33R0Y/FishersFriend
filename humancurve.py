...
class HumanCurve():
    ...
    def generateInternalKnots(self, leftBoundary, rightBoundary, downBoundary, upBoundary, knotsCount):
        ...
        if not isinstance(knotsCount, int) or knotsCount < 0:
            raise ValueError("knotsCount must be a non-negative integer")
        ...
        # Convert boundaries to integers
        leftBoundary = int(leftBoundary)
        rightBoundary = int(rightBoundary)
        downBoundary = int(downBoundary)
        upBoundary = int(upBoundary)
